...
void FaroShuffle::SetParams(vector<int> params) {}
...
void CutDeck::SetParams(vector<int> params)
{
    if (params.size() == 1)
    {
        pos = params[0];
    ...
    throw std::invalid_argument("Not enough parameters entered for this type of shuffle");
}
...
void DealPiles::SetParams(vector<int> params)
{
    if (params.size() == 2)
    {
        numPiles = params[0];
        cardsPerPile = params[1];
        return;
    }
    throw std::invalid_argument("Not enough parameters entered for this type of shuffle");
}
...
{
    cout << "{\"Type\":\"Deal Poker Hands\",\"NumberOfPiles\":\"" << numPiles << "\","
...
void OverhandShuffle::SetParams(vector<int> params)
{
    cardsPerShuffle = params;
}
...
void DealClumps::SetParams(vector<int> params)
{
    cardsPerDeal = params;
}
...
CardShuffle *CardShuffleGenerator(int key)
{
    ...
    int shuffleSelect = key == -1 ? GetRandomInt(0, COUNT - 1) : key;

    switch (shuffleSelect)
    {
    case (FAROSHUFFLE):
